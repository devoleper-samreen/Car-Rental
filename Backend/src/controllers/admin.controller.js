...
import moment from "moment"; // install if not done: npm i moment
  ...
  try {
    ...
    const thisMonthRevenue = await Booking.aggregate([
      {
        $match: {
          status: "confirmed",
          pickupDate: { $gte: thisMonthStart.toDate() },
        },
      },
      {
        $group: {
    ...
    return res.status(200).json({
      ...
      stats: {
        ...
        changePercent: {
          users: +3.6, // static for now
          cars: -7.4, // static for now
          bookings: -52.6, // static for now
