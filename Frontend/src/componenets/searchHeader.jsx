...
function searchHeader({ onSearch }) {
  ...
  return (
    <div className="max-w-[1100px] mx-auto h-24 bg-white flex items-center p-4 justify-between">
      <div>
        <Link to="/" className="text-3xl font-bold text-[#4F3CF7]">
          CarsNow
        </Link>
      </div>
      ...
      <div
        className="w-12 h-12 rounded-full bg-white/20 text-purple-500 flex items-center justify-center text-lg font-bold hover:bg-white/30 transition-all cursor-pointer border-[2px] border-purple-500"
        title={user?.name}
        onClick={() => {
